...
const ShareDialog = ({ content, type }: ShareDialogProps) => {
  const platforms = [
    ...
    },
    {
      ...
      icon: Linkedin,
      share: () => {
        const url = `https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(window.location.href)}&title=${encodeURIComponent(`EchoVerse ${type} post`)}&summary=${encodeURIComponent(content)}`;
        window.open(url, '_blank');
      }
    }
  ];
